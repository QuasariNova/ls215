...
function isItemAvailable(id, transactions) {
  totalQuantity = transactionsFor(id, transactions)
    .map(({movement, quantity}) => movement === 'in' ? quantity : -quantity)
    .reduce((sum, value) => sum + value); // we could roll map into reduce
  return totalQuantity > 0;
}

function transactionsFor(inventoryID, list) {
  return list.filter(({id}) => id === inventoryID);
}
