...
function sentiment(text) {
  // first need to replace all none words with spaces:
  text = text.toLowerCase().replace(/[^a-z']+/g, ' ');
  // Split into array of words
  let words = text.split(' ');

  // filter words to positive and negative
  let positive = words.filter(word => positiveWords.includes(word));
  let negative = words.filter(word => negativeWords.includes(word));

  console.log(`There are ${positive.length} positive words in the text.`);
  console.log(`Positve sentiments: ${positive.join(', ')}`);
  console.log('');
